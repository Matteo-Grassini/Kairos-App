...
export const DayDetailModal: React.FC<DayDetailModalProps> = ({ 
    date, onClose, schedule, tasks, meals, workouts, onBatchAction, onRemoveTask, onRemoveEvent
}) => {
                            ...
                            <div className="space-y-3 relative">
                                ...
                                {schedule.map((item, i) => (
                                    <div key={i} className="flex gap-4 relative items-start">
                                        ...
                                        <div className="flex-1 bg-white border border-slate-100 p-3 rounded-xl shadow-sm flex justify-between items-center gap-3">
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start mb-1">
                                                    <h4 className="font-bold text-slate-800 text-sm truncate">{item.title}</h4>
                                                    <span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-1.5 py-0.5 rounded whitespace-nowrap">
                                                        {item.startTime} - {item.endTime}
                                                    </span>
                                                </div>
                                                {item.location && (
                                                    <div className="flex items-center gap-1 text-[10px] text-slate-500 mb-1 truncate">
                                                        <MapPin size={10} /> {item.location}
                                                    </div>
                                                )}
                                            </div>
                                            {/* Delete Button for Fixed Event */}
                                            {item.type === 'fixed' && (
                                                <TrashButton onClick={() => onRemoveEvent(item.id)} />
                                            )}
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                {tasks.map((task, i) => (
                                    <div key={i} className="flex items-center gap-3 p-3 bg-slate-50/50 rounded-xl border border-slate-100">
                                        ...
                                        <TrashButton onClick={() => onRemoveTask(task.id)} />
                                    </div>
                                ))}
                            </div>
                                    ...
                                    {meals.map((meal, i) => (
                                        <div key={i} className="flex items-center justify-between p-3 bg-emerald-50/30 rounded-xl border border-emerald-100">
                                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="font-bold text-sm text-slate-800 truncate">{meal.title}</h4>
                                                    <p className="text-[10px] text-slate-500">{meal.startTime} • {meal.nutritionalInfo?.calories || '-'} kcal</p>
                                                </div>
                                            </div>
                                            <TrashButton onClick={() => onRemoveEvent(meal.id)} />
                                        </div>
                                    ))}
                                </div>
                                    ...
                                    {workouts.map((wo, i) => (
                                        <div key={i} className="flex items-center justify-between p-3 bg-orange-50/30 rounded-xl border border-orange-100">
                                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="font-bold text-sm text-slate-800 truncate">{wo.title}</h4>
                                                    <p className="text-[10px] text-slate-500">{wo.startTime} • {wo.workoutInfo?.muscleGroup || 'Generico'}</p>
                                                </div>
                                            </div>
                                            <TrashButton onClick={() => onRemoveEvent(wo.id)} />
                                        </div>
                                    ))}
                                </div>
