...
const getEventStyle = (item: ScheduleItem) => {
    ...
    if (item.category === 'meal' || t.match(/pranzo|cena|colazione|snack|mangiare/)) {
        if(t.match(/caffè|break/)) return { bg: 'bg-amber-100/90 backdrop-blur-md', border: 'border-l-4 border-amber-400', text: 'text-amber-900', subtext: 'text-amber-700', icon: Coffee };
        return { bg: 'bg-emerald-100/90 backdrop-blur-md', border: 'border-l-4 border-emerald-400', text: 'text-emerald-900', subtext: 'text-emerald-700', icon: Utensils };
    }
    
    if (item.category === 'work' || t.match(/riunione|meet|call|lavoro|ufficio/)) {
        if(t.match(/mail|scrivere/)) return { bg: 'bg-blue-100/90 backdrop-blur-md', border: 'border-l-4 border-blue-400', text: 'text-blue-900', subtext: 'text-blue-700', icon: Mail };
        return { bg: 'bg-blue-100/90 backdrop-blur-md', border: 'border-l-4 border-blue-400', text: 'text-blue-900', subtext: 'text-blue-700', icon: Briefcase };
    }
    
    if (item.category === 'workout' || t.match(/palestra|allenamento|corsa|yoga/)) {
        return { bg: 'bg-orange-100/90 backdrop-blur-md', border: 'border-l-4 border-orange-400', text: 'text-orange-900', subtext: 'text-orange-700', icon: Dumbbell };
    }
    
    if (item.category === 'study' || t.match(/studio|leggere|libro|esame/)) {
        return { bg: 'bg-violet-100/90 backdrop-blur-md', border: 'border-l-4 border-violet-400', text: 'text-violet-900', subtext: 'text-violet-700', icon: BookOpen };
    }

    if (item.type === 'fixed') {
         return { bg: 'bg-slate-100/90 backdrop-blur-sm', border: 'border-l-4 border-slate-400', text: 'text-slate-800', subtext: 'text-slate-500', icon: Lock };
    }

    // Generic / Task
    return { bg: 'bg-indigo-50/90 backdrop-blur-md', border: 'border-l-4 border-indigo-400', text: 'text-indigo-900', subtext: 'text-indigo-700', icon: Zap };
};

export const Timeline: React.FC<TimelineProps> = ({ schedule, summary, isLoading, onEventClick, currentDate }) => {
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto relative custom-scrollbar scroll-smooth">
        <div className="relative w-full py-10" style={{ height: 24 * HOUR_HEIGHT }}> 
          ...
          {isToday && (
            <div 
                ...
                style={{ top: (new Date().getHours() * 60 + new Date().getMinutes()) * PIXELS_PER_MINUTE }}
            >
          ...
          {layoutItems.map((item) => {
            ...
            return (
              <div
                ...
                className={`absolute rounded-xl overflow-hidden hover:scale-[1.01] hover:shadow-lg hover:z-40 transition-all duration-300 cursor-pointer border-l-4 ${style.bg} ${style.border}`}
                style={{ 
                    top: `${top}px`, 
                    height: `${height - 2}px`, 
                    left: `${leftOffset}px`,
                    width: width,
                    zIndex: 10 + item.indentationLevel 
                }}
              >
                <div className="flex flex-col h-full p-1.5 lg:p-2 relative z-10">
