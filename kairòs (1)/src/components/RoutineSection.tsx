...
import { Plus, Clock, Sun, Sunset, Moon, Repeat, Loader2, Trash2, CheckCircle2, Paperclip, Settings, X, Save, UploadCloud, ChevronDown, ChevronUp, Zap, Sunrise, Check, Circle, Trophy, Heart, Smile, Meh, Battery, Book, Sparkles, AlarmClock, Pencil } from 'lucide-react';
import { FixedEvent, Task, EventCategory, UploadedFile, TaskType, WellnessEntry, MoodType, RoutineGoals } from '../types';
...
interface RoutineSectionProps {
  ...
  // Wake Up Time Props
  wakeUpTime: string;
  onUpdateWakeUpTime: (time: string) => void;
  // Goals Props
  routineGoals: RoutineGoals;
  onUpdateGoals: (goals: RoutineGoals) => void;
}
...
export const RoutineSection: React.FC<RoutineSectionProps> = ({ 
    ...
    wakeUpTime, onUpdateWakeUpTime,
    routineGoals, onUpdateGoals
}) => {
  ...
  // Goal Editing State (BUFFERED)
  const [isEditingGoals, setIsEditingGoals] = useState(false);
  const [tempGoals, setTempGoals] = useState<RoutineGoals>(routineGoals);

  // Sync temp goals only when opening edit mode
  useEffect(() => {
      if (!isEditingGoals) {
          setTempGoals(routineGoals);
      }
  }, [routineGoals, isEditingGoals]);
                  ...
                  {stats.progress >= 100 ? (
                      ...
                      <>
                        <div className="flex items-baseline gap-1">
                            ...
                            {isEditingGoals ? (
                                <input type="number" value={tempGoals.habitsGoal} onChange={e => setTempGoals({...tempGoals, habitsGoal: Number(e.target.value)})} className="w-10 text-lg font-bold text-slate-400 bg-transparent border-b border-indigo-200 text-center outline-none pointer-events-auto" />
                            ) : (
                    ...
                    {activeTab === 'habits' && (
                        <>
                            {isEditingGoals ? (
                                ...
                                <button 
                                    onClick={() => { setTempGoals(routineGoals); setIsEditingGoals(true); }}
                    ...
                    <div className="flex-1 w-full">
                        ...
                        <div className="w-full flex flex-col justify-center h-full gap-4">
                            ...
                            <div className="bg-white p-3 rounded-2xl border border-slate-100 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <div className="p-1.5 bg-orange-100 text-orange-600 rounded-lg"><AlarmClock size={16} /></div>
                                    <span className="text-xs font-bold text-slate-600">Sveglia</span>
                                </div>
                                <input type="time" value={wakeUpTime} onChange={(e) => onUpdateWakeUpTime(e.target.value)} className="bg-slate-50 border-none rounded-lg px-2 py-1 font-bold text-slate-800 outline-none text-sm w-24 text-center focus:ring-2 focus:ring-orange-200" />
                            </div>
                            ...
                            <div className={`bg-slate-50 p-3 rounded-2xl border ${isEditingGoals ? 'border-indigo-200 ring-1 ring-indigo-100' : 'border-slate-100'}`}>
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    {isEditingGoals ? (
                                        <div className="flex items-center gap-1"><input type="number" value={tempGoals.sleepHours} onChange={e => setTempGoals({...tempGoals, sleepHours: Number(e.target.value)})} className="w-10 text-xs font-black bg-transparent border-b border-slate-300 text-center outline-none" /><span className="text-xs font-bold text-slate-700">h</span></div>
                            ...
                            <div className="flex gap-2 w-full justify-end">
                                ...
                                <button onClick={() => planInputRef.current?.click()} disabled={importingPlan} className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-bold text-slate-600 hover:border-indigo-300 hover:text-indigo-600 transition-all">
                                    {importingPlan ? <Loader2 size={12} className="animate-spin" /> : <UploadCloud size={12} />} Importa Routine
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={`bg-white rounded-3xl shadow-xl shadow-slate-200/40 border border-white overflow-hidden transition-all duration-500 ${isInputOpen ? 'ring-4 ring-slate-50' : ''}`}>
                        {!isInputOpen ? (
                            ...
                            <div className="p-6 animate-in slide-in-from-top-4">
                                <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Zap size={20} className="text-indigo-500" /> Habit Builder</h3><button onClick={() => setIsInputOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400"><ChevronUp size={20} /></button></div>
                                <div className="flex gap-2 overflow-x-auto no-scrollbar mb-6 pb-2"><QuickAddItem label="Meditazione" /><QuickAddItem label="Lettura" /><QuickAddItem label="Journaling" /><QuickAddItem label="Stretching" /></div>
